    ...
    if "W" in k:
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                params_tmp = copy.deepcopy(params_orig)
                params_tmp[k][i, j] += eps
                out = forward(xb, params_tmp, "layer1", sigmoid)
                probs = forward(out, params_tmp, "output", softmax)
                loss1, _ = compute_loss_and_acc(yb, probs)

                params_tmp = copy.deepcopy(params_orig)
                params_tmp[k][i, j] -= eps
                out = forward(xb, params_tmp, "layer1", sigmoid)
                probs = forward(out, params_tmp, "output", softmax)
                loss2, _ = compute_loss_and_acc(yb, probs)
